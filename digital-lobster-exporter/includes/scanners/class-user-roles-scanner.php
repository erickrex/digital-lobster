...
class Digital_Lobster_User_Roles_Scanner {
    ...
    public function scan() {
        ...
        foreach ($roles as $role_slug => $role_data) {
            $role_info = array(
                ...
                'is_default' => in_array($role_slug, $this->default_roles),
                'is_custom' => !in_array($role_slug, $this->default_roles),
    ...
    /**
     * Export user roles data to JSON file
     *
    ...
    public function export($export_dir) {
        ...
        if ($json === false) {
            return new WP_Error('json_encode_failed', 'Failed to encode user roles data to JSON');
        }
        
        $file_path = trailingslashit($export_dir) . 'user_roles.json';
        $result = file_put_contents($file_path, $json);
        
        if ($result === false) {
            return new WP_Error('file_write_failed', 'Failed to write user_roles.json');
        }
